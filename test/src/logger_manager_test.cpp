...
TEST(LoggerManagerTest, CreateLoggerWithDefaultConfiguration) {
  // Arrange
  LoggerManager logger_manager;
  auto config = logger_manager.GetConfiguration();

  // Act
  auto logger = logger_manager.CreateLogger("logger");
...
TEST(LoggerManagerTest, CreateLoggerWithChangedConfigurationRegisterFalse) {
  // Arrange
  LoggerManager logger_manager;
  auto json = json::JSON({
                           {"root", "/"},
